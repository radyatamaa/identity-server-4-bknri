...
namespace IdentityServer4.Test
{
    ...
    public class TestUserResourceOwnerPasswordValidator : IResourceOwnerPasswordValidator
    {
        ...
        {
            if (await _usersService.ValidateCredentials(context.UserName, context.Password,context.LoginType,context.UserType))
            {
                var user = await _usersService.FindByUsername(context.UserName, context.LoginType, context.UserType);

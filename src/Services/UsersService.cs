...
namespace IdentityServer4.Serivces
{
    public class UsersService : IUsersService
    {
        ...
        {
            var getUserByusername = await GetByUsername(username,loginType,userType);
        ...
        public async Task<Users> GetByUsername(string username,string loginType, int userType)
        {
            await Task.Yield();
            var user =  _dbContext.Set<Users>().Where(o => o.Username == username && o.IsActive == true &&
            o.IsDeleted == false && o.LoginType == loginType && o.UserType == userType).FirstOrDefault();
            if (user != null)
            {
                if(isDecrypt == false) {
                    var password = this.Decrypt(user.Password);
        ...
        public async Task<bool> VerifiedEmail(VerifiedOTP verifiedOTP)
        {
            ...
            var user = new OTPTemp();
            if (!string.IsNullOrEmpty(verifiedOTP.Email))
            {
                user = _dbContext.Set<OTPTemp>().Where(o => o.Email == verifiedOTP.Email && o.OTP == verifiedOTP.CodeOTP && o.Expired >= now).FirstOrDefault();
            }
            else if (!string.IsNullOrEmpty(verifiedOTP.PhoneNumber))
            {
                user = _dbContext.Set<OTPTemp>().Where(o => o.PhoneNumber == verifiedOTP.PhoneNumber && o.OTP == verifiedOTP.CodeOTP && o.Expired >= now).FirstOrDefault();
            }
